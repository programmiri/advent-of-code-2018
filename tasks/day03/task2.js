  ...
  return input
    ...
    .reduce((acc, curr) => {
      acc.push({
        ...
        squares: parseInt(curr[3]) * parseInt(curr[4])
      });
...
function getSingleClaimedRecs(gridObj) {
  return Object.keys(gridObj).reduce((acc, curr) => {
    const rec = gridObj[curr];
    if (rec.claimedBy.length == 1) {
      acc.push(rec);
    }
    return acc;
  }, []);
